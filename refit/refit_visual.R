...
ggsave("refit/figures/visual_refit.pdf", width = 10, height = 10)
...
do_final_cpue <- function(mu, s) {
  ...
  inseason <- read_csv(cpue_source)
  # Add ccpue col
  inseason <- inseason |> mutate(ccpue = cumsum(cpue))
...
ggsave("refit/figures/visual_refit_finalcpue.pdf", width = 10, height = 10)
